...
import { AlertTriangle, Bug, ExternalLink, HelpCircle, RefreshCw } from 'lucide-react'
...
export function DeploymentError({
  ...
  return (
    <main
      className={cn('deployment-section flex flex-col text-center', className)}
    ...
    >
      ...
      <div className='deployment-card-enhanced deployment-status-error'>
        <div className='space-y-4'>
          <div>
            <h2 id='deployment-error-title' className='deployment-text-title text-foreground'>
              {errorCategory.title}
            </h2>
        ...
        <div className='deployment-card-enhanced'>
          <details className='text-left'>
            ...
            <div className='deployment-text-caption font-mono bg-muted/50 p-3 rounded-lg overflow-auto max-h-40'>
              <pre className='whitespace-pre-wrap text-xs'>{errorStack}</pre>
            </div>
          </details>
        </div>
      )}
...
export function CompactDeploymentError({
  ...
  return (
    ...
    >
      ...
      <span className='deployment-text-body flex-1 text-left'>{errorMessage}</span>
      {onRetry && (
        <ActionButton
          intent='secondary'
          onClick={onRetry}
