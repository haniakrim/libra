...
export function UrlPreview({ url, status, showActions = true, className }: UrlPreviewProps) {
  ...
  return (
    <div
      className={cn('deployment-card-enhanced transition-all duration-300 border', className)}
      style={{
    ...
    >
      ...
      <div className='flex items-center justify-between gap-3'>
        <a
          ...
          aria-label={
            status === 'live'
              ? m['ide.deployment.urlPreview.openWebsite']({ url })
              : m['ide.deployment.urlPreview.previewAddress']({ url })
          }
        >
          <span
            className={cn(
        ...
        {showActions && (
          <Button
            ...
            aria-label={
              copied
                ? m['ide.deployment.urlPreview.copiedToClipboard']()
                : m['ide.deployment.urlPreview.copyLinkToClipboard']()
            }
            aria-live='polite'
          >
            {copied ? (
